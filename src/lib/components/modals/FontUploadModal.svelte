<script lang="ts">
    ...
    async function addFontsFromFiles(files: FileList | null | undefined) {
        ...
        if (!type || !["font/ttf", "font/otf", "font/woff", "font/woff2"].includes(type.mime)) {
            return;
        }
...
<Modal title="Upload a font" bind:this={fontUploadModal} key="O">
    {#if step === 1}
        <label
            ...
            <p class="mx-auto mt-3 w-fit text-sm text-white/70">(.ttf, .otf, .woff2 recommended)</p>
            <input class="hidden" type="file" id="fontFileUpload" onchange={selectHandler} />
        </label>
    {:else if step === 2}
        <p>
        ...
        <button onclick={addToFontLUT} class="w-fit rounded-md bg-zinc-900 p-2 hover:bg-black/50">
            Add font
        </button>
    {/if}
</Modal>
