<script lang="ts">
    ...
    import { colourMap, defaultColorLUT, trueMarkOrUndefined } from "$lib/text/utils";
    import { addTypeSpecificValues } from "$lib/text/nbt/export";
    import { FontsExtension, Obfuscation, ShadowColorMark } from "$lib/tiptap/extensions/index";
    import TextStyleButtons from "../toolbar/TextStyleButtons.svelte";
        ...
        {#if editor}
            ...
            {#each colourMap as color}
                <button
                    ...
                    title={color.name}
                    class="rounded-md p-0.5 text-sm hover:bg-white/3 {editor.isActive('textStyle', {
                        color: color.value,
                    })
                        ? 'bg-zinc-800'
                        : ''}"
            ...
            <label for="color" class="rounded-md p-0.5 text-sm font-medium hover:bg-white/3"
                ><IconColor /></label>
            <input
                type="color"
                id="color"
