...
import {
	isAnInteractiveProp,
	isDefinedTextObject as isATextComponent,
} from "../utils";
...
function getSharedStyleProps(
	...
	for (const prop of styleProps) {
		...
		if (
			a[key] !== undefined &&
			b[key] !== undefined &&
			propertiesMatch(a[key], b[key], prop)
		) {
	...
	return [
		sharedStyleProps,
		Object.keys(sharedStyleProps) as (keyof MinecraftText)[],
	];
}
...
function collectStyledRun(
	...
	let j = i;
	let sharedStyleKeys = Object.keys(
		sharedStyleProps,
	) as (keyof MinecraftText)[];
...
function mergeTextComponents(output: StringyMCText[]) {
	...
	for (let i = 0; i < output.length - 1; i++) {
		...
		if (typeof current === "object" && typeof next === "object") {
			const [sharedProperties, sharedPropertyKeys] = getSharedStyleProps(
				current,
				next,
			);
